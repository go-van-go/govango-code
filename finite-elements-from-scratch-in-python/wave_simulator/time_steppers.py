...
class LowStorageRungeKutta:
    ...
    def _compute_time_step_size_xijun(self):
        ...
        c = self.physics.max_speed
        alpha = 0.400 # max is 0.503 for eta1 = 0.33 and eta2 = 0.87
        dt = (alpha * d) / c
        # correct dt for integer # of time steps
        self.num_time_steps = int(np.ceil(self.t_final/ dt))
        self.dt = (self.t_final / self.num_time_steps)

    def _apply_source_term(self, K_p, Kbar_p, tau):
        x0, y0, z0 = (0.5, 0.5, 0.00)
        source_radius = 0.019
        source_frequency = 8
        amplitude = 40
        ...
        source_nodes = np.where(
            ...
            (np.sqrt((x - x0)**2 + (y - y0)**2) < source_radius)
        )
        dim = 2
        K_p = K_p.ravel(order='F')
        Kbar_p = Kbar_p.ravel(order='F')
        K_p[source_nodes] = amplitude * np.sin(2 * np.pi * source_frequency * (self.t + tau * self.dt))
        Kbar_p[source_nodes] = amplitude * np.sin(2 * np.pi * source_frequency * (self.t + (1 - tau) * self.dt))
        K_p = K_p.reshape((self.physics.mesh.reference_element.nodes_per_cell, self.physics.mesh.num_cells), order='F')
        Kbar_p = Kbar_p.reshape((self.physics.mesh.reference_element.nodes_per_cell, self.physics.mesh.num_cells), order='F')
        return K_p, Kbar_p
    ...
    def advance_time_step_rk_with_force_term(self):
        ...
        # apply source term
        K_p, Kbar_p = self._apply_source_term(K_p, Kbar_p, tau)

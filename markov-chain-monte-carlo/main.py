...
def find_spring_params():
    ...
    initial_likelihood = np.sum((y_synthetic - y_true_sampled) ** 2)
    
    # initialze arrays to keep track of likelihood and
    # spring parameter guesses
    likelihood = np.zeros(n)
    inputs = np.zeros((2, n))
    inputs[:, 0] = [k, c]
    old_likelihood = initial_likelihood
    likelihood[0] = old_likelihood
    
    # initial guess for spring parameters
    k_guess, c_guess = 3, 0.40
    
    # Run Metropolis-Hastings iteration
    for current_run in range(1, n):
        # propose next guesses for k and c
        kp = abs(k_guess + w * (2 * np.random.rand() - 1))
        cp = abs(c_guess + w * 0.1 * (2 * np.random.rand() - 1))
        ...
        new_likelihood = np.sum((y_synthetic - y_proposed_sampled) ** 2)
        ...
        #plot_data(current_run, inputs, y_true, y_proposed, t, m)
...
def plot_best(inputs, y_true, k_guess, c_guess, m, t, time_of_samples, y_synthetic):
    ...
    plt.legend()
    plt.title('Current y_proposed vs yTrue', **csfont)
...
def plot_data(current_run, inputs, y_true, y_proposed, t, m):
    ...
    plt.subplot(2, 2, 1, facecolor=bg_color)
    plt.scatter(inputs[0, :current_run], inputs[1, :current_run], color=font_color, alpha=0.3)
    plt.xlabel('k', color=font_color)
    plt.ylabel('c', color=font_color)
    plt.title('Scatter plot of k vs. c', color=font_color)
    plt.tick_params(axis='both', colors=axis_color)
    ...
    # y_proposed vs yTrue
    plt.subplot(2, 2, 2, facecolor=bg_color)
    plt.plot(t, y_true, label='True Solution', color="#b48ead")
    plt.plot(t, y_proposed, label='Current Guess', color="#ebcb8b")
    plt.legend(facecolor=bg_color, labelcolor=font_color, loc="upper right")
    plt.title('Current y_proposed vs y_true', color=font_color)
    plt.ylim(-2.0, 2.0)
    plt.tick_params(axis='both', colors=axis_color)
    plt.gca().spines['top'].set_color(border_color)
    plt.gca().spines['right'].set_color(border_color)
    ...
    plt.subplot(2, 2, 3, facecolor=bg_color)
    plt.hist(inputs[0, :current_run], bins=bins, color="#5e81ac", edgecolor=edge_color)
    plt.xlabel('k values', color=font_color)
    plt.title('Histogram of k', color=font_color)
    plt.tick_params(axis='both', colors=axis_color)
    plt.gca().spines['top'].set_color(border_color)
    plt.gca().spines['right'].set_color(border_color)
    plt.gca().spines['left'].set_color(border_color)
    ...
    plt.subplot(2, 2, 4, facecolor=bg_color)
    plt.hist(inputs[1, :current_run], bins=bins, color="#bf616a", edgecolor=edge_color)
    plt.xlabel('c values', color=font_color)
    plt.title('Histogram of c', color=font_color)
    plt.tick_params(axis='both', colors=axis_color)
    plt.gca().spines['top'].set_color(border_color)
    plt.gca().spines['right'].set_color(border_color)
    plt.gca().spines['left'].set_color(border_color)
    ...
    # Save the figure
    plt.tight_layout()
    plt.savefig(f'mcmc_images/frame_{current_run:04d}.png')
    plt.close()
